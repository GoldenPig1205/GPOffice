...
namespace GPOffice.Modes
{
    class DoubleUp
    {
        ...
        public async Task OnModeStarted()
        {
            ...
            Player.List.ToList().ForEach(x => x.Broadcast(10, $"<size=25><b>[<color=#{Plugin.Mods[mod1].ToString().Split('/')[0]}>{mod1}</color> + <b><color=#{Plugin.Mods[mod2].ToString().Split('/')[0]}>{mod2}</color>]</b></size>"));
        }
    }
}
